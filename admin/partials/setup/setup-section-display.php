...
<?php echo '<p>'.esc_html('Already have an account?  Login to your account ').'<a target="_blank" href="https://app.enzuzo.com">'.esc_html('here').'</a>.</p>' ?>
<?php echo '<h3>'.esc_html('Where to find the installation code snippet or UUID?').'</h3>' ?>
<?php echo '<p>'.esc_html('You can find the UUID (looks like aaaaaaaa-bbbb-cccc-dddd-eeeeeeeeeeee) or JavaScript snippet from this ').'<a target="_blank" href="https://app.enzuzo.com/settings/cookiebar/add-to-website">'.esc_html('path').'</a> in the Enzuzo Admin Dashboard.' ?>
